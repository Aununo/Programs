...
ConstDef: Y_ID Y_ASSIGN ConstInitVal {$$ = newVarDecl(get_conststype(type), type, 1, $1, NULL, $3);}
         | Y_ID ConstExps Y_ASSIGN ConstInitVal {$$ = newVarDecl(get_conststype(type), type, 1, $1, $2, $4); }
         ; 

ConstExps: Y_LSQUARE ConstExp Y_RSQUARE {$$ = $2;}
         | Y_LSQUARE ConstExp Y_RSQUARE ConstExps {$2->next = $4; $$ = $2; }
         ;

ConstInitVal: ConstExp
             | Y_LBRACKET Y_RBRACKET {$$ = NULL; }
             | Y_LBRACKET ConstInitVal Y_RBRACKET {$$ = $2;}
             | Y_LBRACKET ConstInitVal ConstInitVals Y_RBRACKET {if($2 != NULL) {$2->next = $3;} $$ = $2; }
             ;
...
ConstInitVals: Y_COMMA ConstInitVal {$$ = $2;}
              | Y_COMMA ConstInitVal ConstInitVals {if($2 != NULL) {$2->next = $3;} $$ = $2; }
              ;
...
VarDef: Y_ID {$$ = newVarDecl(get_stype(type), type, 0, $1, NULL, NULL);}
       ...
       | Y_ID ConstExps {$$ = newVarDecl(get_stype(type), type, 0, $1, $2, NULL); }
       | Y_ID ConstExps Y_ASSIGN InitVal {$$ = newVarDecl(get_stype(type), type, 0, $1, $2, $4); }
       ; 

InitVal: Exp
        | Y_LBRACKET Y_RBRACKET {$$ = NULL;}
        | Y_LBRACKET InitVal Y_RBRACKET {$$ = $2;}
        | Y_LBRACKET InitVal InitVals Y_RBRACKET {if($2 != NULL) {$2->next = $3;} $$ = $2; }
        ;

InitVals: Y_COMMA InitVal {$$ = $2; }
         | Y_COMMA InitVal InitVals {if($2 != NULL) {$2->next = $3;} $$ = $2; }
         ;
...
LVal: Y_ID {$$ = newDeclRefExp($1, NULL, NULL);}
    | Y_ID ArraySubscripts {$$ = newDeclRefExp($1, $2, NULL);}
    ;
